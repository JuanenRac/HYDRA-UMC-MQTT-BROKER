...
MQTT 3.1.1 標準を実装しており(Aedes 経由、実機で検証済み - 下記アーキテクチャ参照)、最小限のオーバーヘッドで高効率なデータ
...
* 🪪 **クライアント認証：** 任意の MQTT CONNECT ユーザー名/パスワード認証（`MQTT_AUTH_JSON`）により、ACL に検証済みのセッション ID を提供します——各認証情報自身が要求する `clientIdPrefix` も強制されるため、有効だが権限の低い認証情報が、より高い権限を持つ別のクライアント ID を単純に選んでその ACL 権限を継承することはできません。*(実装済み。ACL と組み合わせて使用)*
* 📏 **ペイロードサイズ制限：** PUBLISH のペイロードサイズに対する実際のオプトイン方式の上限で、`MAX_PAYLOAD_BYTES` で設定可能です。*(実装済み)*
* ⚡ **WebSocket サポート：** ブラウザベースのクライアント向けの MQTT over WebSocket——同じブローカーインスタンス上で、既存のプレーン TCP と並ぶ本物のオプトイン第2リスナー(`wsPort` オプション / `MQTT_WS_PORT` 環境変数、デフォルトポート 8083)。*（実装済み）*
* ⏳ **retained メッセージの有効期限：** retained メッセージ向けの本物のオプトイン TTL(`retainedTtlMs` オプション / `MQTT_RETAINED_TTL_MS` 環境変数)——制限より古い retained 状態は、以後すべての購読者に永久に配信され続けるのではなく、実際に消去される。*（実装済み）*
...
* **`src/server.ts` が意図的に薄い理由。** Aedes が実際の MQTT プロトコル処理（CONNECT/PUBLISH/SUBSCRIBE のフレーミング、QoS、保持メッセージと遺言メッセージ）を担当します。このファイルは Aedes が必要とする素の TCP トランスポート、オプションの MQTT-over-WebSocket リスナー、ACL/認証/ペイロード上限の配線、そしてプロセスレベルのロギングだけです。`buildBroker()` はエクスポートされており、テストが一時ポートで実際のブローカーを起動し、実際の MQTT クライアントを接続できます。
...
* **このブローカーが MQTT v5 ではなく MQTT 3.1.1 を話す理由。** `aedes@1.1.1`(固定された依存関係)は MQTT 3.1/3.1.1 のみを実装しています——実際に `protocolVersion: 5` を指定した本物の `mqtt` クライアントを接続して検証したところ、ブローカーは能動的にこれを拒否しました(`Connection refused: Unacceptable protocol version`)。これは Aedes 自身のアップストリームドキュメント(MQTT 5.0 サポートは未リリースの別ブランチにある)とも一致します。このエコシステムのすべてのクライアント(`mqtt_transport.py` 系ブリッジ、`Vda5050Publisher`、本リポジトリ自身のテスト)はすでに 3.1.1 でしかネゴシエートしていないため、これは動作変更ではなくドキュメントの修正です。
...
├── dist/        # バンドル出力（npm run build -> dist/server.cjs、gitignored）
├── images/      # メディアと図表
